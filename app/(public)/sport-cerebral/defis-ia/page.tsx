...
export const metadata = { title: "Défi IA contre humain — Sport cérébral" };

type Level = "facile" | "moyen" | "difficile";
const VALID: Level[] = ["facile", "moyen", "difficile"];

export default async function DefisIaPage({ searchParams }: { searchParams: { niveau?: string } }) {
  const niveau = (VALID.includes(searchParams.niveau as Level) ? searchParams.niveau : "facile") as Level;
  const game = (await getEffectiveGame("defis-ia"))!;
  const { allowed, access } = await getGameAccess("defis-ia");
  ...
  return (
    <section className="section py-10">
      ...
      <h1 className="mt-3 text-3xl font-extrabold tracking-tight text-foreground">{game.title} — Morpion</h1>
