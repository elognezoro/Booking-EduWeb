...
export const metadata: Metadata = { title: "Certificat de réussite · CERTEL Niveau 3" };
...
export default async function CertelN3CertificatPage() {
  const user = await getCurrentUser();

  if (!user) {
    return (
      <section className="formation-scope section py-16">
        <Link href="/certel/niveau-3" className="mb-6 inline-flex items-center gap-1 text-sm font-medium text-muted-foreground hover:underline"><ArrowLeft className="size-4" /> Niveau 3</Link>
        <div className="mx-auto max-w-md rounded-2xl border border-border bg-card p-8 text-center shadow-soft">
          ...
          <p className="mt-2 text-muted-foreground">Connectez-vous à votre compte pour générer votre certificat de réussite du Niveau 3 à votre nom.</p>
          <Button asChild className="mt-5"><Link href="/login?callbackUrl=/certel/niveau-3/certificat"><LogIn className="size-4" /> Se connecter</Link></Button>
        </div>
      </section>
    );
  }
  ...
  const cert = await getOrCreateCertelCertificate(user.id, user.fullName, "N3");
  const cfg = await getCertelCertConfig("N3");
