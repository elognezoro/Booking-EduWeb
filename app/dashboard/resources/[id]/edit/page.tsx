...
export default async function EditResourcePage({ params }: { params: { id: string } }) {
  ...
  const [resource, categories, sites, departments, managers] = await Promise.all([
    prisma.resource.findFirst({ where: { id: params.id, organizationId } }),
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5">
      <Link href={`/dashboard/resources/${resource.id}`} className="inline-flex items-center gap-1.5 text-sm font-semibold text-muted-foreground hover:text-foreground">
        <ArrowLeft className="size-4" /> Retour
      </Link>
      <PageHeader title="Modifier la ressource" description={resource.name} />
      <ResourceForm action={action} categories={categories} sites={sites} levels={levels} services={services} managers={managers} resource={resource} />
    </div>
  );
}
